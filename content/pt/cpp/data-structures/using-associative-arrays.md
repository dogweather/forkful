...
aliases: - /pt/cpp/using-associative-arrays.md
...
Arrays associativos, conhecidos como `std::map` ou `std::unordered_map` em C++, preenchem a lacuna entre índices de arrays e dados do mundo real, permitindo que você use chaves significativas. Eles são a escolha ideal quando você precisa de buscas, inserções e deleções rápidas usando chaves em vez de posições de índice.
