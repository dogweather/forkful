---
changelog:
- 2024-03-17, gpt-4-0125-preview, translated from English
date: 2024-03-17 21:46:43.394966-06:00
description: "\u0E01\u0E32\u0E23\u0E2A\u0E01\u0E31\u0E14\u0E2A\u0E48\u0E27\u0E19\u0E22\
  \u0E48\u0E2D\u0E22\u0E02\u0E2D\u0E07\u0E2A\u0E15\u0E23\u0E34\u0E07\u0E2B\u0E21\u0E32\
  \u0E22\u0E16\u0E36\u0E07\u0E01\u0E32\u0E23\u0E14\u0E36\u0E07\u0E2A\u0E48\u0E27\u0E19\
  \u0E17\u0E35\u0E48\u0E40\u0E09\u0E1E\u0E32\u0E30\u0E40\u0E08\u0E32\u0E30\u0E08\u0E07\
  \u0E02\u0E2D\u0E07\u0E2A\u0E15\u0E23\u0E34\u0E07\u0E2D\u0E2D\u0E01\u0E21\u0E32\u2014\
  \u0E25\u0E33\u0E14\u0E31\u0E1A\u0E02\u0E2D\u0E07\u0E2D\u0E31\u0E01\u0E02\u0E23\u0E30\
  \u0E20\u0E32\u0E22\u0E43\u0E19\u0E2A\u0E15\u0E23\u0E34\u0E07\u0E17\u0E35\u0E48\u0E43\
  \u0E2B\u0E0D\u0E48\u0E01\u0E27\u0E48\u0E32\u2026"
lastmod: '2024-03-17T21:57:56.069272-06:00'
model: gpt-4-0125-preview
summary: "\u0E01\u0E32\u0E23\u0E2A\u0E01\u0E31\u0E14\u0E2A\u0E48\u0E27\u0E19\u0E22\
  \u0E48\u0E2D\u0E22\u0E02\u0E2D\u0E07\u0E2A\u0E15\u0E23\u0E34\u0E07\u0E2B\u0E21\u0E32\
  \u0E22\u0E16\u0E36\u0E07\u0E01\u0E32\u0E23\u0E14\u0E36\u0E07\u0E2A\u0E48\u0E27\u0E19\
  \u0E17\u0E35\u0E48\u0E40\u0E09\u0E1E\u0E32\u0E30\u0E40\u0E08\u0E32\u0E30\u0E08\u0E07\
  \u0E02\u0E2D\u0E07\u0E2A\u0E15\u0E23\u0E34\u0E07\u0E2D\u0E2D\u0E01\u0E21\u0E32\u2014\
  \u0E25\u0E33\u0E14\u0E31\u0E1A\u0E02\u0E2D\u0E07\u0E2D\u0E31\u0E01\u0E02\u0E23\u0E30\
  \u0E20\u0E32\u0E22\u0E43\u0E19\u0E2A\u0E15\u0E23\u0E34\u0E07\u0E17\u0E35\u0E48\u0E43\
  \u0E2B\u0E0D\u0E48\u0E01\u0E27\u0E48\u0E32\u2026"
title: "\u0E01\u0E32\u0E23\u0E14\u0E36\u0E07\u0E02\u0E49\u0E2D\u0E21\u0E39\u0E25\u0E22\
  \u0E48\u0E2D\u0E22\u0E2D\u0E2D\u0E01\u0E21\u0E32"
---

{{< edit_this_page >}}

## อะไร & ทำไม?

การสกัดส่วนย่อยของสตริงหมายถึงการดึงส่วนที่เฉพาะเจาะจงของสตริงออกมา—ลำดับของอักขระภายในสตริงที่ใหญ่กว่า นักพัฒนาโปรแกรมตัดและแยกสตริงเพื่อตรวจสอบข้อมูลที่ป้อน, แยกความหมายของข้อมูล, หรือเพียงเพื่อแสดงเฉพาะส่วนที่เกี่ยวข้องกับผู้ใช้เท่านั้น

## วิธีการ:

การสกัดส่วนย่อยของสตริงใน Java ทำได้ง่ายโดยใช้เมทอด `substring` นี่คือวิธีการทำ:

```java
public class SubstringExample {
    public static void main(String[] args) {
        String fullString = "Hello, World!";

        // สกัดจากดัชนีที่ 7 จนถึงตอนท้ายของสตริง
        String sub1 = fullString.substring(7);
        System.out.println(sub1); // ผลลัพธ์: World!

        // สกัดจากดัชนีที่ 0 จนถึงดัชนีที่ 4 (5 ไม่รวม)
        String sub2 = fullString.substring(0, 5);
        System.out.println(sub2); // ผลลัพธ์: Hello
    }
}
```

**จำไว้**: ใน Java, การนับดัชนีสตริงเริ่มต้นที่ 0

## ศึกษาเพิ่มเติม

เมทอด `substring` มีมาตั้งแต่เริ่มต้นของ Java, ให้วิธีการง่ายๆ ในการสกัดส่วนของสตริง เวอร์ชันเก่าๆ ของ Java, `substring` จะแชร์อาร์เรย์อักขระต้นฉบับซึ่งอาจนำไปสู่การรั่วไหลของหน่วยความจำหากสตริงต้นฉบับมีขนาดใหญ่และส่วนย่อยถูกเก็บไว้เป็นเวลานาน นับตั้งแต่ Java 7 อัพเดต 6, `substring` สร้างสตริงใหม่, ดังนั้นสตริงเก่าสามารถถูกเก็บขยะได้หากไม่ถูกใช้งานในที่อื่น

นอกจากนี้, ก่อนที่จะใช้ `substring`, พิจารณาว่าคุณสามารถใช้ `split`, `replace`, หรือยูทิลิตี้ regex สำหรับสถานการณ์ที่ซับซ้อนกว่าได้หรือไม่ ภายใน, `substring` ใน Java ใช้เมทอดจากคลาส `String` ที่คัดลอกอาร์เรย์—มีประสิทธิภาพ, แต่ไม่ใช่สิ่งที่คุณมีการควบคุมโดยตรง

## ดูเพิ่มเติม

- เพื่อภาพรวมที่ครบถ้วนว่าคุณสามารถทำอะไรกับสตริงใน Java, ดูเอกสารคลาส `String`: [String (Java SE 15 & JDK 15)](https://docs.oracle.com/en/java/javase/15/docs/api/java.base/java/lang/String.html)
- ดำดิ่งไปในการจัดการสตริงที่ซับซ้อนยิ่งขึ้น? คลาส `Pattern` และ `Matcher` เป็นเพื่อนของคุณ: [Pattern (Java SE 15 & JDK 15)](https://docs.oracle.com/en/java/javase/15/docs/api/java.base/java/util/regex/Pattern.html)
- บทช่วยสอนเกี่ยวกับการใช้ regular expressions ใน Java: [Regular Expressions](https://docs.oracle.com/javase/tutorial/essential/regex/)

ไม่ว่าจะเป็นการตัดที่รวดเร็วหรือการสกัดข้อมูลที่ซับซ้อน, ฟังก์ชันที่คุณต้องการมีอยู่ที่นั่น เข้าใจและเตรียมชุดเครื่องมือของคุณให้พร้อมอยู่เสมอ
